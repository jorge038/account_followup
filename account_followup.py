...
class res_partner(osv.osv):
    ...
    def do_partner_mail(self, cr, uid, partner_ids, context=None):
        '''
        This function opens a window to compose an email, with template message loaded by default

        all validations (if !"compose_message" in ctx:),  are for send mails when
        all letters is generated in automatic
        '''
        if context is None:
            context = {}
        ctx = context.copy()
        ...
        for partner in self.browse(cr, uid, partner_ids, context=ctx):
            if partner.email and partner.email.strip():
                ...
                if "compose_message" not in ctx:
                    ...
                    raise osv.except_osv(_('Warning'), payment_next_action)
        if "compose_message" not in ctx:
            return unknown_mails
        try:
        ...
        ctx.update({
            ...
            'default_use_template': bool(mail_template_id[1]),
            'default_template_id': mail_template_id[1],
